...
import (
	...
	"github.com/ville-vv/eth-chain-store/src/common/go-eth/p2p/nodestate"
)
...
func newBalanceTracker(ns *nodestate.NodeStateMachine, setup *serverSetup, db ethdb.KeyValueStore, clock mclock.Clock, posExp, negExp utils.ValueExpirer) *balanceTracker {
	...
	// calculate the total number of issued server tokens.
...
// TotalTokenAmount returns the current total amount of server tokens in existence
